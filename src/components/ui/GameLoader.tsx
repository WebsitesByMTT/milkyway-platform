...
interface GameLoaderProps {
  loadingpercent: number; // Percentage of loading progress
  setLoadingPercent: (value: number) => void; // Function to update loading percentage
}
...
const GameLoader: React.FC<GameLoaderProps> = ({
  ...
  useEffect(() => {
    ...
    function createStars(numStars: number): void {
    ...
    // Function to create animated elements in the viewport
    function createElements(numElements: number): void {
    ...
    function drawStars(): void {
      ...
      if (starsContainer) starsContainer.appendChild(tmpStar);
    }
      ...
      <div className="flex items-center justify-center gap-[2vw] w-[70%] z-[1000]">
        ...
        <div className="relative flex items-center w-[70%] h-[3vh] sm:h-[2vw]">
          <input
            ...
            max={100}
            step={1}
            onChange={(e) => setLoadingPercent(Number(e.target.value))}
            className="slider"
            disabled
            id="gameLoader"
          />
